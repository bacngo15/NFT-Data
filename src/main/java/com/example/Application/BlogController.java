...
public class BlogController extends SwitchLayout implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        timeButton.setOnMouseClicked(event -> TextFieldTimeSearch());
    ...
    public void TextFieldTimeSearch() {
        String searchTerm = timeText.getText();
        SearchByTime(searchTerm, list);
    }
    ...
    public void SearchByTime(String s, List<Article> blog) {
        ObservableList<Article> blogList = FXCollections.observableList(blog);
        ObservableList<Article> searchByTimeList = FXCollections.observableArrayList();
        for (Article blogs : blogList) {
            String date = blogs.getDate();
            if (date.toLowerCase().contains(s.toLowerCase())) {
                searchByTimeList.add(blogs);
            }
            initializeTableView(searchByTimeList);
        }
    }
}
